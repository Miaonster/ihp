...
//lets get it started... 
function doLoad(){
    $("#wrapper, #footer").delay(1000).fadeIn("slow");
    //check for update
...
function get_root_dirs(){
    ...
    if (os == "win") {currentDrives = air.File.getRootDirectories();}
    else {currentDrives = new air.File('/Volumes/').getDirectoryListing();}
    
    for (i = 0; i < currentDrives.length; i++) {
		//build drive links
		$("#file_list").append("<p><a class='drive_letters' href='#' onclick='setdirectory(this);'>"+currentDrives[i].name+"</a></p>");
		
    }
    ROOT_DRIVES = currentDrives;
}
function setdirectory(elem){
	//alert($(elem).text());
	
	$(elem).css({"background-color":"#f00", "color":"#fff", "cursor":"inherit"}).delay(1000).css({"background-color":"#ccc", "color":"#000", "cursor":"inherit"});
	
	var drive_path = "";

	if (os == "win") {
        drive_path = $(elem).text();
    }
	else{ drive_path = '/Volumes/'+$(elem).text();}
